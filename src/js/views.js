...
export default function createNeuralNetworkWorkspace(parentElement, config) {
  ...
  function mouseOverTools(x, y, toolNumber) {
    return toolButtonPositions(x, y, toolNumber).reduce(
      (selectedIndex, pos, index) => {
  ...
  function mouseOverSynapseButtons(x, y, dendritePositions) {
    return synapseClusterPositions(x, y, dendritePositions, [], false).reduce(
      (selectedIndex, pos, index) => {
        if (
          mouseOverCircle(pos.buttonX, pos.buttonY, config.synapse.buttonRadius)
        ) {
          return index;
        }
        return selectedIndex;
   ...
   */
  function synapseClusterPositions(
    axonX,
    axonY,
    dendritePositions,
    actionPotentials,
    ...
    const startStopAverage = dendritePositions.reduce(
      (total, pos) => {
        return {
    ...
    return dendritePositions.map(pos => {
      ...
      const buttonPosition = quadraticBezier(
        axonX,
        axonY,
        ctrlX,
        ctrlY,
  ...
  function drawNeuron(x, y, inhibitory, membranePotential, adjusting) {
    context.fillStyle = adjusting
      ? config.color.adjusting
      : inhibitory
      ? config.color.inhibitory
      : config.color.excitatory;
    context.beginPath();
    context.arc(
      x * scale + offsetX,
      y * scale + offsetY,
  ...
  function drawSynapse(startX, startY, ctrlX, ctrlY, stopX, stopY, color) {
    ...
    const ctrlRadius =
      (config.synapse.startRadius + config.synapse.stopRadius) / 2;
